...
import Data.Semigroup (Max (..), Min (..), Semigroup (sconcat))
...
instance (FromJSON a) => FromJSON (Interval UTCTime a) where
    parseJSON = withObject "Interval" $ \v ->
        Interval
            <$> (parseIso =<< v .: "begin")
            <*> (parseIso =<< v .: "end")
            <*> v .: "value"
...
instance FromJSON IntervalFile where
    parseJSON = withObject "IntervalFile" $ \v ->
        IntervalFile
            <$> (parseIso =<< v .: "start")
            <*> (parseIso =<< v .: "finish")
...
encodeIntervals moment b xxs@(x : xs) =
    encode . Just $
        IntervalFile
            (getMin (sconcat (NE.map (Min . begin) (x :| xs))))
...
decodeFile p =
    either error return . eitherDecode =<< case p of
        "-" -> BL.getContents
