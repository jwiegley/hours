...
instance
    (t ~ UTCTime, u ~ NominalDiffTime, Show a) =>
    Show (Budget t u a)
    where
    show Budget{..} =
        let v = variantToLisp
         in concat
                ...
                , ")\n"
                , "(ideal-total            . "
                , v (DiffTimeVal bIdealTotal)
                , ")\n"
                , "(ideal-expected         . "
                , v (DiffTimeVal bIdealExpected)
                , ")\n"
                , "(ideal-remaining        . "
                , v (DiffTimeVal bIdealRemaining)
                , ")\n"
                , "(ideal-expected-exact   . "
                , v (DiffTimeVal bIdealExpectedExact)
                , ")\n"
                , "(ideal-remaining-exact  . "
                , v (DiffTimeVal bIdealRemainingExact)
                , ")\n"
                , "(ideal-days-left        . "
                , v (IntVal bIdealPeriodsLeft)
                , ")\n"
                , "(ideal-days-left-incl   . "
                , v (IntVal bIdealPeriodsLeftIncl)
                , ")\n"
                , "(real-completed         . "
                , v (DiffTimeVal bRealCompleted)
                , ")\n"
                , "(real-remaining         . "
                , v (DiffTimeVal bRealRemaining)
                , ")\n"
                , "(real-expected          . "
                , v (DiffTimeVal bRealExpected)
                , ")\n"
                , "(real-expected-inact    . "
                , v (DiffTimeVal bRealExpectedInact)
                , ")\n"
                , "(real-this-completed    . "
                , v (DiffTimeVal bRealThisCompleted)
                , ")\n"
                , "(real-this-remaining    . "
                , v (DiffTimeVal bRealThisRemaining)
                , ")\n"
                , "(current-period         . "
                , v (OtherVal bCurrentPeriod)
                , ")\n"
                , "(current-period-span    . "
                , v (DiffTimeVal bCurrentPeriodSpan)
                , ")\n"
                , "(expectation            . "
                , v (DiffTimeVal bExpectation)
                , ")\n"
                ]
...
calculateBudget start finish now base now' loggedIn ideal def real =
  ...
  where
    ...
    bRealExpected
        | loggedIn, Just _ <- current = activeExpectation
        | otherwise = bRealExpectedInact
    bRealExpectedInact
        | null future = bRealRemaining
        | otherwise =
            bRealRemaining
